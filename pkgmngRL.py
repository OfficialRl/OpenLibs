# Corregido unos errores tanto de sintáxis (dejando dos renglones en algunas partes) y alguno que otro de ortografía.
...
class Ansi:
    ...
    def cprint(self, content, color) -> int:
        if content.endswith('/not_newline'):
    ...
    def cmenu(self, options) -> int:
        ...
        for option in range(len(options)):
            self.cprint('[/not_newline' + 'green')
            num_str = option + '/not_newline'
            self.cprint(num_str, 'orange')
            self.cprint(None, '] ~ /not_newline' + 'green')
...
class CheckData():
    ...
    def check_url_exist(self, url) -> bool:
    ...
    def check_repo_exits(self, url) -> bool:
        ...
        if is_https != 'https://':
            url += 'https://'
        exists = self.check_url_exist(url)
        if exists == True:
            return True
        else:
            return False
                        ...
                        if is_valid == True:
                            ...
                            while True:
                                ...
                                if valid_commit_name == True:
                                    ...
                                    Ansi.cprint('Invalid commit name ~ Cannot include spaces')
                        else:
